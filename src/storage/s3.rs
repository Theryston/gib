...
static CAPABILITY_TEMPORARY_FILE_COUNTER: AtomicU64 = AtomicU64::new(0);
...
impl S3FS {
    pub fn new(config: S3FSConfig) -> Result<Self, String> {
        let region = config
            .region
        ...
        let access_key = config
            .access_key
            .ok_or_else(|| "Access key is required".to_string())?;
        let secret_key = config
            .secret_key
            .ok_or_else(|| "Secret key is required".to_string())?;
...
fn persist_capabilities(path: Option<&Path>, capabilities: S3Capabilities) {
    ...
    let temporary = path.with_file_name(format!(
        ...
        CAPABILITY_TEMPORARY_FILE_COUNTER.fetch_add(1, Ordering::Relaxed)
    ));
